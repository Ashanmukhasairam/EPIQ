...
def lambda_handler(event, context):
    ...
    if request_type == "DescribeConnectorConfigurationRequest":
        return {
            ...
            "authenticationConfig": {
                ...
                "oAuth2Defaults": None,
                "customAuthConfig": [
                    {"authenticationType": "NO_AUTH", "authParameters": []}
                ]
            },
            ...
            "operatorsSupported": [
                ...
                "VALIDATE_NON_ZERO", "VALIDATE_NON_NEGATIVE",
    ...
    # ========================================================
    # 3. Validate Connector Runtime Settings
    # ========================================================
    ...
    # ========================================================
    # 6. Query Data — WITH PAGINATION (50 per page)
    # ========================================================
        ...
        page_size  = 50                                    # ← 50 per page
        offset     = int(next_token) if next_token else 0

        logger.info(f"[ENV] {env} | [ENTITY] {entity} | [OFFSET] {offset} | [PAGE SIZE] {page_size}")
        ...
        try:
            ...
            # ── ContractList records ──
            if entity_key == "contractlist":
                raw_records = body.get("contract_list", {}).get("value", [])
                logger.info(f"ContractList page: {len(raw_records)} records")
            ...
            # ── ContractDetails records ──
                ...
                logger.info(f"ContractDetails page: {len(raw_records)} records")
                ...
                for contract in raw_records:
                    records.append(json.dumps({
                        ...
                        "ContractItems":                 contract.get("_ContractItems", [])
                    }))

            # ── Next token ──
            new_next_token = str(offset + page_size) if has_more else None

            logger.info(f"Returning {len(records)} records | has_more={has_more} | next_token={new_next_token}")
