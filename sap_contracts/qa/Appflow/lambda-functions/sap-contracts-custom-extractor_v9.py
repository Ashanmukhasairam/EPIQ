...
CONTROL_TABLE          = os.environ.get("CONTROL_TABLE","edp_incremental_control")
...
def update_control_table_watermark(
    ...
    log_info(
        "WATERMARK: Updating DynamoDB",
        table_name=table_name,
        new_watermark=new_watermark
    )

    try:
        dynamo_audit_table.put_item(
            Item={
                "etl_run_id": "WATERMARK",
                "layer": f"CONTROL#{table_name}",
                "last_successful_watermark": new_watermark,
                "updated_at": datetime.utcnow().isoformat(),
                "last_run_id": etl_run_id
            }
        )

        log_info(
            "WATERMARK: Successfully updated in DynamoDB",
            new_watermark=new_watermark
        )
        ...
        log_error(
            "WATERMARK: Failed to update DynamoDB",
            error=str(e)
        )
        return False
...
def lambda_handler(event, context):
    ...
    CONTROL_TABLE = "edp_incremental_control"
