...
from pyspark.sql.functions import (
    ...
    explode, row_number, md5, concat_ws, from_json
)
...
def read_mapping(base_object: str):
    ...
    if not rows:
        raise RuntimeError(
            ...
            f"source_system='{SOURCE}', source_object='{base_object}'."
        )
...
def read_bronze(folder: str, etl_run_date):
    ...
    try:
        ...
        # ============================================================
        # HANDLE STRINGIFIED JSON ("raw_json")
        # ============================================================
        if "raw_json" in df.columns:
            ...
            # Convert string → structured JSON
            df = df.withColumn(
                "parsed",
...
def already_processed(base_object: str) -> bool:
    """
    Returns True if this etl_run_date already has active rows.
    Used ONLY to control audit update — merge always runs.
    """
    ...
    if count > 0:
        logger.info(
            ...
            f"for etl_run_date={ETL_RUN_DATE}. Audit will be skipped."
        )
        return True
...
#     # ── Explode contract_items from nested JSON ───────────────
...
#             .select(
#                 col("ContractNumber"),
#                 explode(col("ContractItems")).alias("item"),
#             )
#             .select(col("ContractNumber"), col("item.*"))
#         )
#         post = bronze_df.count()
#         logger.info(f"  Exploded contract_items: {pre:,} → {post:,} rows")
...
#     # ── Dedup within batch ────────────────────────────────────
#     # contract_header → single PK (contract_number)
#     # contract_items  → composite PK (contract_number + item_number)
#     w_dedup      = Window.partitionBy(*pk_cols).orderBy(lit(1))
...
#     # Include ALL mapped columns in the hash (PK + business)
...
def transform_data(
    ...
    if base_object == "contract_items":
        ...
        # FIX: normalize exploded columns to lowercase
        bronze_df = bronze_df.toDF(*[c.lower() for c in bronze_df.columns])
...
def process_object(
    ...
    if not spark.catalog.tableExists(table_ref):
        ...
        logger.info("  Table exists — incremental SCD2 run")

    # ── Idempotency flag — audit control only ─────────────────
    # Merge always runs — hash check handles no-ops efficiently
    ...
    logger.info(f"  was_already_processed : {was_already_processed}")
...
try:
    ...
    # ── Read bronze once — shared by both tables ──────────────
    # Both contract_header and contract_items come from
    # the same contractdetails bronze folder
    bronze_df, rows_read = read_bronze("contractdetails", ETL_RUN_DATE)

    if bronze_df is None or rows_read == 0:
        logger.warning("  No bronze data found. Skipping all tables.")
        for table in tables:
            update_audit(
                table_name=table,
                rows_read=0,
        ...
        # Cache bronze — reused for both header and items
        ...
        for table in tables:
            ...
            if table not in ("contract_header", "contract_items"):
                raise ValueError(f"Unrecognised SILVER_TABLE value '{table}'.")

            tbl, rows_read, rows_written, records_rejected, was_already_processed = process_object(
                table, bronze_df, rows_read
            )

            # ── Audit: skip on rerun, update on first run ─────
            if was_already_processed:
                logger.info(
                    f"  Rerun detected for {table} — "
                    f"audit not updated. Original metrics preserved."
                )
            else:
                update_audit(
                    table_name=tbl,
    ...
    failed_table = table if "table" in locals() else "UNKNOWN"
