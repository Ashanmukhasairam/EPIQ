...
def insert_audit(layer, tables, job_name, **context):
    ...
    for table in tables:
        layer_table = f"{layer}#{table}"  # e.g. SILVER#sap#contract_header
        ...
        if existing:
            log.info(f"Audit record already exists for {layer_table} — skipping insert.")
            continue
        ...
        log.info(f"Audit record inserted for {layer_table}")
