# ============================================================
# SAP SILVER GLUE JOB contract_list
# ============================================================
...
def read_bronze(folder, etl_run_date):
    ...
    path = f"s3://{BRONZE_BUCKET}/{SOURCE}/contracts/{folder}/{year}/{month}/{day}/"
...
def transform_data(bronze_df, mapping_rows, base_object, pk_cols):
    ...
    window = Window.partitionBy(*(pk_cols + ["contract_time"])).orderBy(
...
try:
    ...
    if SILVER_TABLE == "contract_list":
        bronze_df, rows_read = read_bronze(f"contractlists",ETL_RUN_DATE)

    else:
        raise ValueError(f"Invalid table {SILVER_TABLE}")
    
    table_name, rows_read, rows_written, records_rejected = process_object(SILVER_TABLE, bronze_df, rows_read)
    print("final before audit",table_name, rows_read, rows_written,records_rejected)
    update_audit(table_name=table_name,rows_read=rows_read,rows_written=rows_written,records_rejected=records_rejected,status="SUCCESS")
